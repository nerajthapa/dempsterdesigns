...
contact.html          Contact: photo hero, phone / address / hours / social, form (markup only), CTA band (black)
...
| Contact form | **Formspree** (free tier: 50 submissions/month) or **Netlify Forms** / **Cloudflare Pages Functions + Resend** if you want no third-party branding. | The form is already wired for Formspree; see the comment in `contact.html`. The `_gotcha` honeypot is present; enable reCAPTCHA in the Formspree dashboard if spam appears. |
